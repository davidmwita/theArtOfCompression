...
public:
    ...
    * resulting rectangles. (Ties will be broken by taking the largest
	* of the tied locations (rightmost, or bottommost, depending on splitting dimension)
    *
   ...
   * splitting lines. It will also call helper function buildTree.
   ...
   void renderNode(Node* node, PNG* img);
   ...
   * You may want a recursive helper function for this one.
   ...
   void pruneNode(Node* node, int tol);
   ...
   * You may want a recursive helper function for this one.
   ...
   int pruneSizeNode(Node* node, int tol);
   ...
   int colourDistance(RGBAPixel& c1, RGBAPixel& c2);
   bool shouldPrune (Node* node, Node* ref, int tol);
...
private:
   ...
   * You may want a recursive helper function for this one.
   ...
   /**
		Helper for clear, frees given node (Children are also freed)
   */
   ...
   */
   ...
   /**
		Helper for copy, returns a copy of given node (Children are also copied)
   */
   ...
   */
   ...
   int findVertSplit(stats & s, pair<int,int> ul, pair<int,int> lr);

   int findHoriSplit(stats & s, pair<int,int> ul, pair<int,int> lr);
